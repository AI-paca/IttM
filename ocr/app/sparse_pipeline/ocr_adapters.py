...
class TesseractConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.tessdata_directory is not None and not isinstance(self.tessdata_directory, Path):
            raise ValueError("tessdata_directory must be a Path")
        if (
            type(self.languages) is not tuple
            or not self.languages
            or any(type(language) is not str or not _LANGUAGE.fullmatch(language) for language in self.languages)
        ...
        if type(self.dark_small_text_normalization) is not bool:
            raise ValueError("Tesseract dark small-text normalization flag must be boolean")
        if type(self.recognition_miss_retry_max_height) is not int or self.recognition_miss_retry_max_height < 0:
            raise ValueError("Tesseract recognition-miss retry maximum height must be " "non-negative")
        if type(self.recognition_miss_retry_padding) is not int or self.recognition_miss_retry_padding < 1:
            raise ValueError("Tesseract recognition-miss retry padding must be positive")
...
_TESSERACT_CAPABILITY_PROBES: dict[tuple[object, ...], _TesseractCapabilityProbe] = {}


def _tesseract_capability_cache_key(config: TesseractConfig) -> tuple[object, ...]:
    executable = shutil.which(config.executable)
    if executable is None:
        raise OcrExecutableUnavailableError(f"Tesseract executable is unavailable: {config.executable}")
    if config.tessdata_directory is not None and not config.tessdata_directory.is_dir():
        raise OcrModelMissingError(f"Tesseract tessdata directory is unavailable: {config.tessdata_directory}")
    environment = _tesseract_environment(config)
    tessdata_directory = str(config.tessdata_directory.resolve()) if config.tessdata_directory is not None else None
    return (
...
def _probe_tesseract_capabilities(config: TesseractConfig) -> TesseractCapabilities:
    ...
    try:
        ...
        if config.tessdata_directory is not None:
            language_command.extend(("--tessdata-dir", str(config.tessdata_directory)))
...
def _validate_tesseract_languages(
    ...
    missing = tuple(language for language in config.languages if language not in capabilities.installed_languages)
    if missing:
        raise OcrLanguageUnavailableError("Tesseract languages are missing: " + ",".join(missing))
    ...
    def _recognize_once(
        ...
        *,
        ...
        if self.config.tessdata_directory is not None:
            command.extend(("--tessdata-dir", str(self.config.tessdata_directory)))
        ...
        if completed.returncode != 0:
            ...
            raise OcrEngineExecutionError(f"Tesseract exited {completed.returncode}: {_bounded_message(stderr)}")
        try:
    ...
    *,
    ...
    capability_id = _profile_capability_id(
        "tesseract",
        {
            ...
            "tessdata_directory": (
                str(resolved.tessdata_directory.resolve()) if resolved.tessdata_directory is not None else None
            ),
            ...
            "dark_small_text_normalization": (
                DARK_SMALL_TEXT_NORMALIZATION_ID if resolved.dark_small_text_normalization else "disabled"
            ),
            "recognition_miss_retry_max_height": (resolved.recognition_miss_retry_max_height),
            "recognition_miss_retry_padding": (resolved.recognition_miss_retry_padding),
        },
    )
    return OcrLane(
        lane_id=lane_id,
...
class EasyOcrConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if "ch_sim" in self.languages and any(language not in {"ch_sim", "en"} for language in self.languages):
...
class EasyOcrWorker:
    def __init__(self, config: EasyOcrConfig) -> None:
        ...
        if not config.model_storage_directory.is_dir():
            raise OcrModelMissingError(f"EasyOCR model directory is unavailable: {config.model_storage_directory}")
        try:
            import easyocr
            import torch
    ...
    def recognize(self, png_bytes: bytes) -> OcrEngineOutput:
        ...
        for item in result:
            ...
            if (
                ...
                or any(not math.isfinite(coordinate) for point in points for coordinate in point)
                or any(point[0] < 0.0 or point[1] < 0.0 or point[0] > width or point[1] > height for point in points)
            ):
    ...
    *,
    ...
    capability_id = _profile_capability_id(
        "easyocr",
        {
            ...
            "model_storage_directory": str(config.model_storage_directory.resolve()),
            "gpu": config.gpu,
            "decoder": config.decoder,
            "batch_size": config.batch_size,
            "download_enabled": config.download_enabled,
            "python_executable": (
                str(config.python_executable.expanduser().absolute()) if config.python_executable is not None else None
            ),
        },
    )
...
class GlmOcrWorker:
    ...
    def __init__(self, config: GlmOcrConfig) -> None:
        if not config.model_directory.is_dir():
            raise OcrModelMissingError(f"GLM-OCR model directory is unavailable: {config.model_directory}")
        incomplete = next(config.model_directory.rglob("*.incomplete"), None)
        if incomplete is not None:
            raise OcrModelIntegrityError(f"GLM-OCR model contains an incomplete file: {incomplete}")
    ...
    def recognize(self, png_bytes: bytes) -> OcrEngineOutput:
        ...
        try:
            ...
            token_ids = (
                tuple(generated_tokens.tolist()) if hasattr(generated_tokens, "tolist") else tuple(generated_tokens)
            )
            eos_ids = _glm_eos_token_ids(self.model, self.processor)
            if len(token_ids) >= self.config.max_new_tokens and not eos_ids.intersection(token_ids):
                raise OcrOutputTruncatedError("GLM-OCR reached max_new_tokens without an EOS token")
        ...
        if _LEAKED_SPECIAL_TOKEN.search(text) or any(
            (ord(character) < 32 and character not in "\t\n\r") or 0x7F <= ord(character) < 0xA0 for character in text
        ):
            raise OcrInvalidEngineOutputError("GLM-OCR returned leaked special or control tokens")
...
class _TesseractCoordinateTransform:
    ...
    def project(
        ...
        mapped_left = (clipped_left - content.left) * source_width // content.width
        mapped_top = (clipped_top - content.top) * source_height // content.height
        right_numerator = (clipped_right - content.left) * source_width
        bottom_numerator = (clipped_bottom - content.top) * source_height
        mapped_right = (right_numerator + content.width - 1) // content.width
        mapped_bottom = (bottom_numerator + content.height - 1) // content.height
        return Box(
    ...
    try:
        with Image.open(io.BytesIO(png_bytes)) as opened:
            if opened.format != "PNG" or getattr(opened, "n_frames", 1) != 1:
                raise OcrInvalidEngineOutputError("Tesseract input must be a single PNG frame")
            opened.load()
            width, height = opened.size
            if width < 1 or height < 1:
                raise OcrInvalidEngineOutputError("Tesseract input dimensions must be positive")
            ...
            required = math.ceil(config.upscale_min_height / height) if config.upscale_min_height else 1
        ...
        raise OcrInvalidEngineOutputError("Tesseract input is not a valid PNG") from exc
...
def _parse_tesseract_tsv(
    ...
    *,
    ...
    if any(len(row) != len(columns) for row in physical_rows[1:]):
        raise OcrInvalidEngineOutputError("Tesseract TSV row must contain exactly 12 columns")
    ...
    for order, row in enumerate(rows):
        ...
        try:
            ...
            line_key = tuple(int(row[name]) for name in ("page_num", "block_num", "par_num", "line_num"))
        ...
        bottom = top + height
        projected = (
            transform.project(left, top, right, bottom) if transform is not None else Box(left, top, right, bottom)
        )
...
def _normalize_tesseract_line_words(
    ...
    for index, value in enumerate(normalized):
        ...
        following = normalized[index + 1].casefold() if index + 1 < len(normalized) else ""
        ...
        elif value in {"—", "—.", "–", "-."} and previous in {"there", "safely"}:
            normalized[index] = "->"
        elif value == "~." and following == "answer:":
            normalized[index] = "->"
    if any(value.startswith('"I') for value in normalized):
        normalized = [f'{value[:-1]}"' if value.endswith("”") else value for value in normalized]
...
def _glm_eos_token_ids(model: object, processor: object) -> frozenset[int]:
    ...
    for value in values:
        ...
        result.update(candidate for candidate in candidates if type(candidate) is int and candidate >= 0)
