# Local OCR/VLM deployment scripts

`scripts/ollama-deploy` keeps local Ollama and compatible OCR/VLM deployment
scripts in one device-local workdir. It is intentionally outside the main
application Docker/compose flow so model downloads do not inflate normal
development images.

Tracked files in that directory are only scripts, manifests, and examples.
Real tokens and model caches stay outside git:
