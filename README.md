...
- Локальный Tesseract через Rust-сервис; EasyOCR — через опциональный Python worker.
...
| Local Tesseract | Backend на loopback / Compose                                     | Multipart-`POST` на `http://127.0.0.1:<port>/api/extract/text` | Gateway проксирует тело в `OCR_URL`; Rust пишет upload во временный файл; decode-guard `80MP`; PDF обрабатывается постранично через Poppler. |
...
Rust-сервис сохраняет upload во временный файл и освобождает его после запроса.
Python-сервис совместимости по-прежнему собирает upload в `bytes`.
...
Требуются Node.js 24+, Rust 1.96.1+, pkg-config, Tesseract с development headers
и языками eng/rus, Poppler:
...
Сборка и CLI: [Rust OCR runtime](./ocr-runtime/README.md). Python нужен только
для EasyOCR, проверок сравнения и прежнего сервиса совместимости.

Статическая сборка для работы в браузере:
...
| Rust OCR          | Upload во временном файле; PDF постранично; decode guard `80MP`; отмена при закрытии stream | Один запрос одновременно по умолчанию (`OCR_CONCURRENCY`); изображения и OCR crops находятся в RAM. |
...
| **Backend OCR (Tesseract / EasyOCR)** | Upload во временном файле; декодированная страница и OCR crops в RAM. | Дождаться окончания запроса; временные файлы удаляются автоматически. | Upload до 128 MiB (`OCR_MAX_UPLOAD_BYTES`). Decode guard `80MP`. PDF: `max 100` страниц, `max 6000px` на страницу.                                  |
