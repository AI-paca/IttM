...
from app.sparse_pipeline import v16_sparse_codes as _sparse_codes
...
class RecursiveStopFlag:
    """Externally supplied terminal decision for one recursive crop.

    Stage 1 deliberately does not infer this flag from a table grid, object
    type, or a quality score.  An independent oracle may set it after finding
    a stable character height (or after a blurred-text gauge succeeds).  The
    node path makes the decision replayable and keeps it local to one crop.
    """
...
class RecursiveGridConfig:
    ...
    stop_flags: tuple[RecursiveStopFlag, ...] = ()

    def __post_init__(self) -> None:
        if type(self.stop_flags) is not tuple or any(
            not isinstance(flag, RecursiveStopFlag) for flag in self.stop_flags
        ):
            raise ValueError("recursive stop flags must be an immutable tuple")
        paths = tuple(flag.node_path for flag in self.stop_flags)
        if len(paths) != len(set(paths)):
            raise ValueError("recursive stop paths must be unique")
...
class RegionDecision:
    ...
    character_height_px: int | None = None
    ...
    def metadata(self) -> dict[str, object]:
        ...
        if self.stop_flag:
            value.update(
                {
                    "stop_flag": True,
                    "character_height_px": self.character_height_px,
...
class RecursiveGridAnalysis:
    ...
    nodes: tuple["RecursiveGridNodeTrace", ...] = ()


@dataclass(frozen=True)
class RecursiveGridNodeTrace:
    """One retained call of the literal recursive crop search.

    Earlier adapters kept only the terminal leaves.  That made the recursion
    impossible to replay downstream even though `_segment_node` had actually
    performed it.  Paths are deterministic child ordinals rooted at ``()``.
    """
    ...
    split_coordinate: int | None
    separator_boxes: tuple[Box, ...]
    leaf_index: int | None
    stop_flag: RecursiveStopFlag | None = None


@dataclass
class _RecursiveGridNodeDraft:
    source_bbox: Box
    axis: str | None = None
    child_paths: tuple[tuple[int, ...], ...] = ()
    split_coordinate: int | None = None
    separator_boxes: tuple[Box, ...] = ()
    leaf_key: tuple[Box, Box] | None = None
    stop_flag: RecursiveStopFlag | None = None
...
def analyze_recursive_grid(
    ...
    leaves, nodes = _segment_recursive_grid_with_nodes(image, config)
...
def segment_recursive_grid(
    ...
    leaves, _ = _segment_recursive_grid_with_nodes(image, config)
    return list(leaves)


def _segment_recursive_grid_with_nodes(
    image: Image.Image,
    config: RecursiveGridConfig,
) -> tuple[tuple[RecursiveGridLeaf, ...], tuple[RecursiveGridNodeTrace, ...]]:
    source = image.convert("RGB").copy()
    drafts: dict[tuple[int, ...], _RecursiveGridNodeDraft] = {}
    raw_leaves = _segment_node(
        source,
    ...
    leaves = tuple(
        sorted(
            _deduplicate_leaves(raw_leaves),
            key=lambda leaf: (
                leaf.source_bbox[1],
                leaf.source_bbox[0],
                leaf.source_bbox[3],
    ...
    def retain(path: tuple[int, ...]) -> bool:
        draft = drafts[path]
        child_results = tuple(retain(child) for child in draft.child_paths)
        child_live = any(child_results)
        own_live = draft.leaf_key in leaf_by_key
        if child_live or own_live:
            live_paths.add(path)
            return True
        return False
...
def _segment_node(
    ...
    *,
    ...
    node_path: tuple[int, ...],
    node_drafts: dict[tuple[int, ...], _RecursiveGridNodeDraft],
) -> list[RecursiveGridLeaf]:
    ...
    if depth < config.max_depth and image.height > config.max_region_height:
        ...
        child_paths = tuple(node_path + (index,) for index in range(len(windows)))
        node_drafts[node_path].axis = "rows"
        node_drafts[node_path].child_paths = child_paths
        first_boundary = _map_local_box(
            (0, 0, image.width, windows[0][1]),
            image.size,
            source_bbox,
        )[3]
        node_drafts[node_path].split_coordinate = min(
            source_bbox[3] - 1,
            max(source_bbox[1] + 1, first_boundary),
        )
        for child_index, (top, bottom) in enumerate(windows):
    ...
    node_drafts[node_path].stop_flag = requested_stop
    ...
    if depth < config.max_depth and stop_flag is None:
        ...
        if separator is not None:
            cut, seam_cost, gap_top, gap_bottom = separator
            overlap = 0
            ...
            if (
                ...
                child_paths = (node_path + (0,), node_path + (1,))
                node_drafts[node_path].axis = "rows"
                node_drafts[node_path].child_paths = child_paths
                node_drafts[node_path].separator_boxes = (
                    _map_local_box(
                        (0, gap_top, aligned.width, gap_bottom),
                        aligned.size,
                        source_bbox,
                    ),
                )
        ...
        if vertical_separator is not None:
            ...
            child_paths = (node_path + (0,), node_path + (1,))
            node_drafts[node_path].axis = "columns"
            node_drafts[node_path].child_paths = child_paths
            gutter_left = max(0, cut - gutter_width // 2)
            gutter_right = min(aligned.width, gutter_left + gutter_width)
            node_drafts[node_path].separator_boxes = (
                _map_local_box(
                    (gutter_left, 0, gutter_right, aligned.height),
                    aligned.size,
                    source_bbox,
                ),
            )
