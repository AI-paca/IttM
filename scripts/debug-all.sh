...
if [[ "$pdf_raster" -eq 1 && ${#selected_fixtures[@]} -gt 0 ]]; then
  ...
  if [[ ${#selected_pdf_fixtures[@]} -gt 0 ]]; then
    ...
    mapfile -t raster_outputs < <(
      python3 scripts/debug_pdf_image_probe.py \
        "${selected_pdf_fixtures[@]}" \
...
python3 scripts/debug_matrix_report.py "${matrix_args[@]}"
python3 scripts/debug_quality_gate.py --result "$output_root/result.csv"
