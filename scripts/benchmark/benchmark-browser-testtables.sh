...
(
  cd "$source_root"
  node --import tsx scripts/benchmark/browser-benchmark-profile-json.ts "$browser_profile"
) >"$profile_json"
...
for fixture in "${fixtures[@]}"; do
  ...
  (
    ...
    BROWSER_OCR_PREPROCESS_RUNTIME=node_canvas \
      ...
      node --import tsx scripts/benchmark/benchmark-browser-ocr.ts --profile "$browser_profile" "$fixture"
