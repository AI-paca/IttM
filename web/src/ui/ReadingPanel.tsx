...
export function ReadingPanel({
  ...
  const grammar = useMemo(
    () => analyzeMarkdownGrammar(extractedText),
    [extractedText],
  );

  useEffect(() => {
    const onFullscreenChange = () => {
      setIsFullscreen(document.fullscreenElement === resultRef.current);
    };
  ...
  return (
    <div className="w-full animate-in fade-in duration-700 pb-20 px-0 sm:px-6 md:px-0">
      <article className="w-full text-[var(--color-text-primary)]">
        ...
        {grammarOpen && (
          <section className="mt-4 mx-auto w-full max-w-[1120px] rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] p-4 text-sm text-[var(--color-text-secondary)]">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              >
                {grammar.status === "pass" ? "lint pass" : "lint warnings"}
              </span>
            </div>
            <dl className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
              <GrammarMetric label="Tables" value={grammar.tableCount} />
              <GrammarMetric label="Rows" value={grammar.tableRows} />
              <GrammarMetric label="Max cols" value={grammar.maxColumns} />
              <GrammarMetric label="Merges" value={grammar.mergeMarkers} />
            </dl>
            {[...grammar.errors, ...grammar.warnings].length > 0 && (
              <ul className="mt-3 space-y-1 text-xs">
                {[...grammar.errors, ...grammar.warnings]
                  .slice(0, 8)
                  .map((message) => (
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 pb-6 bg-gradient-to-t from-[var(--color-bg-app)] via-[var(--color-bg-app)]/95 to-transparent md:static md:bg-none md:p-0 md:mt-4 md:flex md:flex-row items-center md:border-t md:border-[var(--color-border-subtle)] md:pt-8 z-30">
          <div className="flex gap-3 w-full mx-auto px-0 md:px-0">
            ...
            <button
              ...
              aria-pressed={grammarOpen}
            >
              <ClipboardCheck className="w-5 h-5 text-[var(--color-text-secondary)]" />
            </button>
            <button
              onClick={onToggleFullscreen}
              className="btn-outline flex md:hidden w-[52px] h-[52px] flex-shrink-0 items-center justify-center"
            ...
            >
              {isFullscreen ? (
                <Minimize2 className="w-5 h-5 text-[var(--color-text-secondary)]" />
              ) : (
...
function GrammarMetric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-bg-inset)] px-3 py-2">
      <dt className="text-[11px] font-bold uppercase tracking-wide text-[var(--color-text-muted)]">
        {label}
      </dt>
      <dd className="mt-0.5 text-lg font-bold text-[var(--color-text-primary)]">
        {value}
      </dd>
    </div>
  );
