...
DOCKER_NOTIFY_SCRIPT="$PROJECT_ROOT/scripts/runtime/notify-docker-restart.sh"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: bash scripts/ci/debug.sh [--clean] [--no-docker] [--no-act]"
      exit 2
      ;;
  esac
done
...
if [ "$RUN_DOCKER" -eq 1 ]; then
  ...
  run_docker_step "generate OCR fixtures inside container" docker compose exec -T ocr python tests/support/quality_fixtures.py
  ...
  run_docker_step "strict backend OCR quality tests" docker compose exec -T -e RUN_OCR_QUALITY=1 ocr pytest tests/quality/test_ocr_quality.py -q
else
