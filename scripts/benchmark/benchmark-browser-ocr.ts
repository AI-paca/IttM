...
function browserProfileFlags(profile: BrowserOcrProfile): string[] {
  const flags = [
    ...
    `spatial_full_page_fallback:${profile.spatialFullPageFallback}`,
    `dark_ui_text_fallback:${profile.darkUiTextFallback}`,
    `contextual_markdown_grammar:${profile.contextualMarkdownGrammar}`,
    ...
    `lexical_correction:${profile.lexicalCorrection}`,
    `ocr_language_retry:${profile.ocrLanguageRetry}`,
    `table_slot_builder:${profile.tableSlotBuilder}`,
    `table_slot_max_columns:${profile.tableSlotMaxColumns}`,
    `recursive_table_cell_ocr:${profile.recursiveTableCellOcr}`,
    `recursive_table_cell_ocr_batch_pixels:${profile.recursiveTableCellOcrBatchPixels}`,
